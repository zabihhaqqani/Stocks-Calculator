...
function submitHandler() {
  ...
  if (
    ...
    outputMsg.innerText = `Enter proper values in the fields!!!!!!`;
  } else {
...
function showMsg(
  ...
  if (current > initial) {
    ...
    outputMsg.innerText = `Yay!! Your Profit is ${profit} and profit Percentage is ${profitPercentage.toFixed(
      1
    )}%`;
  } else if (initial > current) {
    //loss
    outputMsg.style.color = "red";
    outputMsg.innerText = `Oops!! Your Loss is ${loss} and loss Percentage is ${lossPercentage.toFixed(
      1
    )}%`;
  } else {
